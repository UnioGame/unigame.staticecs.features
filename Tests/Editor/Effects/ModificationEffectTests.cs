...
namespace unigame.staticecs.features.Tests {
    ...
    public sealed class ModificationEffectTests {
        ...
        public void SetUp() {
            ...
            new CharacteristicFeature<TestEffectsWorld, SpeedCharacteristic>().RegisterTypes(World<TestEffectsWorld>.Types());
            new ModificationEffectFeature<TestEffectsWorld, SpeedCharacteristic>(registerTickSystem: false)
                .RegisterTypes(World<TestEffectsWorld>.Types());

            World<TestEffectsWorld>.Initialize();
            _tick = new EffectTickSystem<TestEffectsWorld, ModificationEffect<SpeedCharacteristic>>();
        }
        ...
        public void Apply_InstallsModifier_OnExpire_Reverts() {
            ...
            ModificationEffectOperations.Apply<TestEffectsWorld, SpeedCharacteristic>(
                target.GID, source.GID, CharacteristicModifierOp.Mul, 2f, duration: 1f);

            Assert.AreEqual(20f, target.Read<CharacteristicComponent<SpeedCharacteristic>>().Value, 0.0001f);

            Tick(2f);
            _tick.Update();

            Assert.AreEqual(10f, target.Read<CharacteristicComponent<SpeedCharacteristic>>().Value, 0.0001f);
        }
        ...
        public void Reapply_OverwritesModifier_FromSameSource() {
            ...
            ModificationEffectOperations.Apply<TestEffectsWorld, SpeedCharacteristic>(
                target.GID, source.GID, CharacteristicModifierOp.Mul, 2f, duration: 5f);
            Assert.AreEqual(20f, target.Read<CharacteristicComponent<SpeedCharacteristic>>().Value, 0.0001f);

            ModificationEffectOperations.Apply<TestEffectsWorld, SpeedCharacteristic>(
                target.GID, source.GID, CharacteristicModifierOp.Mul, 3f, duration: 5f);
            Assert.AreEqual(30f, target.Read<CharacteristicComponent<SpeedCharacteristic>>().Value, 0.0001f);
        }
        ...
        public void SourceDestroyed_RemovesModifier() {
            ...
            ModificationEffectOperations.Apply<TestEffectsWorld, SpeedCharacteristic>(
                target.GID, source.GID, CharacteristicModifierOp.Mul, 2f, duration: 100f);
            Assert.AreEqual(20f, target.Read<CharacteristicComponent<SpeedCharacteristic>>().Value, 0.0001f);
