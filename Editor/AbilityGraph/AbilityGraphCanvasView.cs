...
namespace unigame.staticecs.features.Editor.AbilityGraph {
    internal sealed class AbilityGraphCanvasView : GraphView {
        ...
        public AbilityGraphCanvasView(Action<AbilityGraphProjection.Node> nodeSelected, Action requestRefresh) {
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt) {
            ...
            if (edgeView != null) {
                AbilityGraphProjection.Edge? edgeData = edgeView.userData is AbilityGraphProjection.Edge edge
                    ? edge
                    : null;
        ...
        public void Render(AbilityAsset asset, AbilityGraphProjection projection) {
            ...
            for (var i = 0; i < _projection.Nodes.Count; i++) {
                ...
                var view = new AbilityGraphNodeView(_asset, this, node, position, HandleNodeSelected);
                _nodes.Add(view);
        ...
        private void DeleteSelectedElements() {
            ...
            for (var i = 0; i < selection.Count; i++) {
                if (selection[i] is AbilityGraphNodeView nodeView) {
                    if (AbilityGraphAssetEditing.RemoveNodeReference(_asset, nodeView.ProjectionNode.Config)) {
                        changed = true;
                    }
                }
                else if (selection[i] is Edge edge && edge.output?.node is AbilityGraphNodeView outputNode) {
                    if (AbilityGraphAssetEditing.DisconnectPort(_asset, outputNode.ProjectionNode.Config, edge.output.portName)) {
                        changed = true;
                    }
                }
            }
            ...
            if (changed) {
                EditorApplication.delayCall -= new EditorApplication.CallbackFunction(_requestRefresh);
                EditorApplication.delayCall += new EditorApplication.CallbackFunction(_requestRefresh);
            }
        }
        ...
        private static TElement FindAncestor<TElement>(VisualElement element) where TElement : VisualElement {
            while (element != null) {
        ...
        private void HandleEdgeAdded(Edge edge) {
            ...
            AbilityGraphAssetEditing.ConnectPort(_asset, outputNodeView.ProjectionNode.Config, edge.output.portName, inputNodeView.ProjectionNode.Config);
        }
        ...
        private void HandleEdgeRemoved(Edge edge) {
            ...
            AbilityGraphAssetEditing.DisconnectPort(_asset, outputNodeView.ProjectionNode.Config, edge.output.portName);
        }
    }
}