...
namespace UniGame.StaticEcs.Features {
    ...
    /// </summary>
    public sealed class ApplyDamageStepActivator<TWorld> : AbilityStepActivatorBase<ApplyDamageStepConfig, TWorld>
        where TWorld : struct, IWorldType {
        protected override StepStatus OnActivate(ApplyDamageStepConfig config, in AbilityStepActivationContext<TWorld> ctx) {
            if (config.Amount <= 0f) {
        ...
        private static StepStatus RaiseAoe(ApplyDamageStepConfig config, in AbilityStepActivationContext<TWorld> ctx) {
        ...
        private static StepStatus Raise(ApplyDamageStepConfig config, EntityGID source, EntityGID target) {
