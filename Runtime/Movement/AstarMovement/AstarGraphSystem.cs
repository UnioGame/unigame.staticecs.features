namespace UniGame.StaticEcs.Features
{
        ...
        where TWorld : struct, IWorldType
    {
        ...
        public void Destroy()
        {
            foreach (
                var entity in World<TWorld>
                    .Query<All<AstarPathComponent, AstarGridGraphComponent>>()
                    .Entities()
            )
            {
        ...
        {
            var backend = AstarGraphUpdateUtility.UpdateBounds<TWorld>(
                graphEntity,
                bounds,
                flushOverride: false
            );
            if (
                backend == null
                || !graphEntity.TryUnpack<TWorld>(out var entity)
