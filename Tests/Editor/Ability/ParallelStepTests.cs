...
namespace unigame.staticecs.features.Tests {
    ...
    public sealed class ParallelStepTests {
        ...
        public void AllSuccess_CompletesWhenAllBranchesComplete() {
            Register(new ParallelStepConfig(new IAbilityStepConfig[] {
        ...
        public void AllSuccess_FailsWhenAnyBranchFails() {
            ...
            var caster = StartCast();
            RunFrame();
            var parentCast = caster.Read<AbilityActiveCastRef>().Cast;
        ...
        public void AnySuccess_CompletesOnFirstSuccess_AndCancelsRemaining() {
            Register(new ParallelStepConfig(
                new IAbilityStepConfig[] {
                    new WaitStepConfig(0f),
        ...
        private static int CountBranchCasts() {
            var count = 0;
            foreach (var _ in World<TestAbilityWorld>
                         .Query<All<AbilityCastRuntimeComponent, AbilityBranchSubcastTag>>()
                         .Entities()) {
                count++;
            }
            return count;
