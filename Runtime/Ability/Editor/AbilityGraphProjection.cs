namespace UniGame.StaticEcs.Features.Editor.AbilityGraph
{
    ...
    internal sealed class AbilityGraphProjection
    {
        ...
        public static readonly AbilityGraphProjection Empty = new(
            Array.Empty<Node>(),
            Array.Empty<Edge>(),
            Array.Empty<string>()
        );

        public AbilityGraphProjection(
        ...
        public static AbilityGraphProjection Build(AbilityAsset asset)
        {
            ...
            var visited = new HashSet<IAbilityStepConfig>(
                ReferenceEqualityComparer<IAbilityStepConfig>.Instance
            );
        ...
        {
            var customTitle = config
                .GetType()
                .GetCustomAttributes(typeof(AbilityStepEditorAttribute), false);
            if (
        ...
        private static string BuildSubtitle(IAbilityStepConfig config)
        {
            return config switch
            {
                ...
                ApplyDamageStepConfig damage =>
                    $"{damage.Type} {damage.Amount:0.##} -> {damage.Mode}",
                ...
                ParallelStepConfig parallel =>
                    $"{parallel.JoinPolicy}, CancelRemaining={parallel.CancelRemainingOnJoin}",
                ConditionalStepConfig conditional =>
                    $"Condition {conditional.Condition?.GetType().Name ?? "None"}",
