namespace UniGame.StaticEcs.Features.Editor.AbilityGraph
{
    ...
    internal sealed class AbilityGraphRuntimeBrowser
    {
        ...
        private void CollectActiveCasts()
        {
            ...
            foreach (
                var entity in World<Main>
                    .Query<All<AbilityCastComponent, AbilityCastOwnerComponent>>()
                    .Entities()
            )
            {
                var runtime = entity.Read<AbilityCastComponent>();
                var owner = entity.Read<AbilityCastOwnerComponent>().Owner;
        ...
        private void CollectEquippedAbilities()
        {
            ...
            foreach (
                var entity in World<Main>
                    .Query<All<World<Main>.Multi<AbilitySlotComponent>>>()
                    .Entities()
            )
            {
                ref readonly var roster =
                    ref entity.Read<World<Main>.Multi<AbilitySlotComponent>>();
                for (var i = 0; i < roster.Length; i++)
                {
                    var abilityId = roster.Get(i).Id;
                    _equippedAbilities.Add(
                        new RuntimeAbilityEntry
                        {
