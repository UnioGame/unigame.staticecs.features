    ...
    internal sealed class AbilityGraphNodePickerWindow : EditorWindow {
        ...
        private void DrawToolbar() {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar)) {
                _filter = GUILayout.TextField(_filter ?? string.Empty, EditorStyles.toolbarSearchField, GUILayout.ExpandWidth(true));
        ...
        private void DrawEntries() {
            if (_entries == null || _entries.Count == 0) {
                EditorGUILayout.HelpBox("No creatable ability node types were discovered.", MessageType.Warning);
                return;
            }
            ...
            for (var i = 0; i < _entries.Count; i++) {
                ...
                shown++;
                if (!string.Equals(currentCategory, entry.Category, StringComparison.OrdinalIgnoreCase)) {
            ...
            if (shown == 0) {
                EditorGUILayout.HelpBox("No node types match the current filter.", MessageType.Info);
            }
        }
        ...
        private bool MatchesFilter(AbilityGraphNodeTypeRegistry.Entry entry) {
            ...
            return entry.SearchText.IndexOf(_filter.Trim().ToLowerInvariant(), StringComparison.Ordinal) >= 0;
        }
    }
}